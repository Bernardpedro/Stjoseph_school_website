    ...
    <div class="max-w-screen-xl mx-auto px-2 sm:px-4 py-2">
      <div class="flex items-center gap-2">
        ...
        <div class="ml-auto flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <button
            type="button"
            class="inline-flex items-center justify-center p-0.5 text-[#1D4ED8] dark:text-blue-400 sm:w-9 sm:h-9 sm:p-0 sm:border sm:border-blue-200 sm:dark:border-blue-800 sm:rounded-lg sm:bg-white sm:dark:bg-gray-800 sm:hover:bg-blue-50 sm:dark:hover:bg-blue-950/50 transition-colors"
            :aria-label="isDarkMode ? t('common.lightMode') : t('common.darkMode')"
            :title="isDarkMode ? t('common.lightMode') : t('common.darkMode')"
            @click="toggleTheme"
          >
            <svg v-if="isDarkMode" class="w-3 h-3 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
            </svg>
            <svg v-else class="w-3 h-3 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
